...
export const EventosAsistidos = () => {
  ...
  const handleDownloadPDF = () => {
    fetch(`http://localhost:3000/usuarios/${usuario.id}/eventos-asistidos/pdf`)
    ...
    <Stack spacing={4}>
      {eventosAsistidos.map((eventoAsistido) => (
        ...
        _hover={{
          ...
          <CardBody>
            <Heading size="md">{eventoAsistido.solicitud.nombre}</Heading>
            <Button mt={4} colorScheme="green" onClick={() => handleOpenModal(eventoAsistido)}>
              Ver Detalles
            </Button>
          </CardBody>
        </Card>
      ...
      {eventoActual && (
        <Modal isOpen={isOpen} onClose={onClose}>
          ...
          <ModalContent>
            ...
            <ModalBody>
              <Text>
                {usuario.nombres} {usuario.apellidos} asistió al evento{" "}
                {eventoActual.solicitud.nombre} el día{" "}
